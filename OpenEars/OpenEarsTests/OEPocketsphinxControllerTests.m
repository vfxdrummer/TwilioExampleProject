...
static NSString * const kExpectationThatRecognitionCanStartAndStopAndWriteCMNOutAndStartAndReadCMNIn = @"Recognition can start, stop, and after stop it writes out a CMN, and then it starts again and can read the CMN back in.";
static NSString * const kExpectationThatRunRecognitionOnWavFileAtPathWorks = @"Confirms \"WORD STATEMENT SOMEONE'S OTHER WORD A PHRASE\" is recognized with a score better than |-10000|";
...
@interface OEPocketsphinxControllerTests : XCTestCase <OEEventsObserverDelegate> {
    ...
    XCTestExpectation *_expectationThatRecognitionCanStartAndStopAndWriteCMNOutAndStartAndReadCMNIn;
...
- (void)testThatRunRecognitionOnWavFileAtPathWorks {
   ...
   //  [[OEPocketsphinxController sharedInstance] setVerbosePocketSphinx:TRUE];
    ...
    [self waitForExpectationsWithTimeout:28
                                 handler:^(NSError *error) {
                                     // handler is called on _either_ success or failure
                                     if (error != nil) {
                                         XCTFail(@"timeout error: %@", error);
...
- (void)testThatRecognitionCanStartAndStopAndWriteCMNOutAndStartAndReadCMNIn {
 
    _expectationThatRecognitionCanStartAndStopAndWriteCMNOutAndStartAndReadCMNIn = [OETestTools setUpExpectationWithDescription:kExpectationThatRecognitionCanStartAndStopAndWriteCMNOutAndStartAndReadCMNIn sender:self];
...
- (void)testThatPocketsphinxCanPerformNormalRecognitionWithNBest {
   ...
   // [OEPocketsphinxController sharedInstance].verbosePocketSphinx = TRUE;
    ...
    [self waitForExpectationsWithTimeout:18
                                 handler:^(NSError *error) {
                                     // handler is called on _either_ success or failure
                                     if (error != nil) {
                                         XCTFail(@"timeout error: %@", error);
...
- (void)testThatSpanishModelDoesNotPickUpExtraneousNoise {
    
    _expectationThatSpanishModelDoesNotPickUpExtraneousNoise = [OETestTools setUpExpectationWithDescription:kExpectationThatSpanishModelDoesNotPickUpExtraneousNoise sender:self];
    
    NSArray *lmArray = @[@"ADIOS",
                         @"LECHUGA",
                         @"MADRID",
                         @"BARCELONA",
                         @"PARIS",
    ...
    NSError *error = [_languageModelGenerator generateLanguageModelFromArray:lmArray  withFilesNamed:@"WordLanguageModel" forAcousticModelAtPath:[[OETestTools environmentAppropriateBundle] pathForResource:@"AcousticModelSpanish" ofType:@"bundle"]];
    
    XCTAssert(error.code == noErr, @"generateLanguageModelFromArray couldn't create the LM from the NSArray containing %@ without returning an error which isn't noErr.",lmArray);
    
    NSString *correctPathToMyLanguageModelFile = [NSString stringWithFormat:@"%@/WordLanguageModel.%@",[NSSearchPathForDirectoriesInDomains(NSCachesDirectory, NSUserDomainMask, YES) objectAtIndex:0],@"DMP"]; 
    NSString *correctPathToMyPhoneticDictionaryFile = [NSString stringWithFormat:@"%@/WordLanguageModel.%@",[NSSearchPathForDirectoriesInDomains(NSCachesDirectory, NSUserDomainMask, YES) objectAtIndex:0],@"dic"];
      [OEPocketsphinxController sharedInstance].vadThreshold = 4.4;
       //[OEPocketsphinxController sharedInstance].verbosePocketSphinx = TRUE;
    //[OELogging startOpenEarsLogging]; 
    [OEPocketsphinxController sharedInstance].pathToTestFile = [[OETestTools environmentAppropriateBundle] pathForResource:@"spanish_short" ofType:@"wav"];    
    [[OEPocketsphinxController sharedInstance] startListeningWithLanguageModelAtPath:correctPathToMyLanguageModelFile dictionaryAtPath:correctPathToMyPhoneticDictionaryFile acousticModelAtPath:[[OETestTools environmentAppropriateBundle] pathForResource:@"AcousticModelSpanish" ofType:@"bundle"] languageModelIsJSGF:FALSE];
    
    [self waitForExpectationsWithTimeout:180
                                 handler:^(NSError *error) {
                                     // handler is called on _either_ success or failure
                                     if (error != nil) {
                                         XCTFail(@"timeout error: %@", error);
...
- (void) pocketsphinxDidStartListening {
    ...
    if([OETestTools currentTestDescriptionIs:kExpectationThatRecognitionCanStartAndStopAndWriteCMNOutAndStartAndReadCMNIn]) {
        if(_cmnTestStarts == 1) {
            ...
            if([OEPocketsphinxController sharedInstance].continuousModel.lastCMNUsed != defaultCMN && [OEPocketsphinxController sharedInstance].continuousModel.lastCMNUsed > 1 && [OEPocketsphinxController sharedInstance].continuousModel.lastCMNUsed < 110) { 
                ...
                [_expectationThatRecognitionCanStartAndStopAndWriteCMNOutAndStartAndReadCMNIn fulfill];
            } else {
