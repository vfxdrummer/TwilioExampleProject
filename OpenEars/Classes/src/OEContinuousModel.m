...
- (void) performSingularStopForDecoder:(ps_decoder_t *)pocketSphinxDecoder {
    if(self.utteranceState != kUtteranceStateEnded) { // We never want to call two end utterances in a row.
...
- (void) checkWhetherJSGFSettingOf:(BOOL)languageModelIsJSGF LooksCorrectForThisFilename:(NSString *)languageModelPath {
    ...
    if([languageModelPath hasSuffix:@".gram"] || [languageModelPath hasSuffix:@".GRAM"] || [languageModelPath hasSuffix:@".grammar"] || [languageModelPath hasSuffix:@".GRAMMAR"] || [languageModelPath hasSuffix:@".jsgf"] || [languageModelPath hasSuffix:@".JSGF"]) {
        ...
        if(!languageModelIsJSGF) { // Probable JSGF file with the ARPA bit set
            if([self openEarsLoggingIsOn]) NSLog(@"The file you've sent to the decoder appears to be a JSGF grammar based on its naming, but you have not set languageModelIsJSGF: to TRUE. If you are experiencing recognition issues, there is a good chance that this is the reason for it.");
        }
        ...
        if(languageModelIsJSGF) { // Probable ARPA file with the JSGF bit set
            if([self openEarsLoggingIsOn]) NSLog(@"The file you've sent to the decoder appears to be an ARPA-style language model based on its naming, but you have set languageModelIsJSGF: to TRUE. If you are experiencing recognition issues, there is a good chance that this is the reason for it.");
        }
...
- (NSDictionary *) setUpCommandArray:(id)commandArrayModel secondItemIsEmpty:(BOOL)secondItemIsEmpty forlanguageModel:(NSString *)languageModelPath dictionaryPath:(NSString *)dictionaryPath acousticModelPath:(NSString *)acousticModelPath isJSGF:(BOOL)isJSGF usingBestPath:(NSNumber *)usingBestpath {
    ...
    BOOL useBesthPath = FALSE;
    
    if(usingBestpath) { // If a usingBestpath value was passed, use it.
        useBesthPath = [usingBestpath boolValue];
    } else { // Otherwise, get it by stealth.
        useBesthPath = ![self dictionaryAtPathIsFromRejecto:dictionaryPath];
    }
    
    NSArray *commandArray = [commandArrayModel commandArrayForlanguageModel:languageModelPath dictionaryPath:dictionaryPath acousticModelPath:acousticModelPath isJSGF:isJSGF usesBestPath:useBesthPath removingNoise:self.removingNoise removingSilence:self.removingSilence vadThreshold:self.vadThreshold secondsOfSilence:self.secondsOfSilenceToDetect secondsOfSpeech:self.secondsOfSpeechToDetect];
...
- (NSInteger) startUtterance {
    return ps_start_utt(self.pocketSphinxDecoder, NULL);
}
...
- (char const *) getHypothesisFromDecoder:(ps_decoder_t *)pocketSphinxDecoder withScore:(SInt32 *)recognitionScore {
    return ps_get_hyp(pocketSphinxDecoder, (int32 *)recognitionScore, NULL);
}

- (SInt32) getProbabilityFromDecoder:(ps_decoder_t *)pocketSphinxDecoder {
    return ps_get_prob(pocketSphinxDecoder, NULL);
}
...
- (char const *)nBestHypothesis:(ps_nbest_t *)nbest withScore:(SInt32 *)recognitionScore {
    return ps_nbest_hyp(nbest, (int32 *)&recognitionScore);
}
...
- (void) getNbestForDecoder:(ps_decoder_t *)pocketSphinxDecoder withHypothesis:(char const *)hypothesis andRecognitionScore:(SInt32)recognitionScore {
...
- (void) runRecognitionOnWavFileAtPath:(NSString *)wavPath usingLanguageModelAtPath:(NSString *)languageModelPath dictionaryAtPath:(NSString *)dictionaryPath acousticModelAtPath:(NSString *)acousticModelPath languageModelIsJSGF:(BOOL)languageModelIsJSGF { // Listen to a single recording which already exists.
    ...
    [self processRaw:[wavData subdataWithRange:NSMakeRange(44, ([wavData length] - 44))]];
...
- (NSError *) stopListening {
    ...
    if(_stopping) {
        NSString *failureReason = @"Error: stopListening was called while a listening session was already in the process of being stopped. Please only call stopListening once and for a listening session which is known to be in progress.";
    ...
    if(self.pocketSphinxDecoder != NULL) {
        while(_processing) { // If any speech is still in a submitted but not returned state, it isn't safe to destroy the Pocketsphinx engine.
